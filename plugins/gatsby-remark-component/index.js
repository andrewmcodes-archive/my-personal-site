const SUPPORTED_COMPONENTS = ["two-columns"]

function fixAST(astNode, level = 0) {
  if (
    astNode.type === "paragraph" &&
    astNode.children &&
    astNode.children.length >= 2
  ) {
    if (
      astNode.children[0].type === "html" &&
      astNode.children[astNode.children.length - 1].type === "html"
    ) {
      let openingTag = astNode.children[0].value.match(/^<([\w-]+)/)
      let closingTag = astNode.children[
      ...
      if (openingTag && closingTag && openingTag[1] === closingTag[1]) {
        ...
        if (SUPPORTED_COMPONENTS.includes(tag)) {
          ...
          astNode.position.end =
            astNode.children[astNode.children.length - 1].position.end
          astNode.position.indent = []
          astNode.value = astNode.children.reduce(
